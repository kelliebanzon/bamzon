...
class LoginVC: UIViewController, DisplayableProtocol {
    ...
    func display() {
        ...
        //TODO: change target to forgot login page
        let forgotButton = FormatUtility.createDefaultButton(withText: "Forgot Password?", withFrame: nil, withCenter: nil)
        forgotButton.addTarget(self, action: #selector(checkLogin), for: .touchUpInside)
        ...
        //TODO: change selector to go to member page.
        // also why is this button necessary? this is the login page
        let memberButton = FormatUtility.createDefaultButton(withText: "Already a member?", withFrame: nil, withCenter: nil)
        memberButton.addTarget(self, action: #selector(checkLogin), for: .touchUpInside)
